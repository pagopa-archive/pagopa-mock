...
<RPT xmlns="http://www.digitpa.gov.it/schemas/2011/Pagamenti/">
	...
	<ibanAddebito>'.$_POST['ibanAddebito'].'</ibanAddebito>
		...
		<datiMarcaBolloDigitale>
			<tipoBollo>'.$_POST['tipoBollo'].'</tipoBollo>
			<hashDocumento>'.$_POST['hashDocumento'].'</hashDocumento>
			<provinciaResidenza>'.$_POST['provinciaResidenza'].'</provinciaResidenza>
		</datiMarcaBolloDigitale>
        </datiSingoloVersamento>
...
if ( $esito === 'OK' ) {
   // $this->flash->success('yes!, everything went very smoothly');
     $log = fopen("../logs/nodoebolloCarrello.log", "a") or die("Unable to open file!");
   fwrite($log,"\n\n\n=========REQUEST CARRELLO BOLLO ===============\n");
   fwrite($log,$REQUEST);
   fwrite($log, "\n\n");
   fwrite($log, $RPT);
   fwrite($log, "\n======= FINE CARRELLO BOLLO ==========");
